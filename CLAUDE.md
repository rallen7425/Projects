...
> **Deploy:** Always run `npx vercel --prod` manually after pushing. GitHub auto-deploy via Vercel is unreliable. Link resolves via `.vercel/repo.json` (not a per-directory `project.json`, which doesn't exist here — that's expected); if a fresh checkout gets "Not authorized," run `npx vercel link --project projects-yg1p --scope rick-allen-s-projects` first (found 2026-08-25).
...
**Phase 16 deployed — Zone Management + Tracking overhaul + Profile reference view (2026-08-22).** Committed (`7c7d684`) and live at https://distilled-news.vercel.app — see Session log.
...
### Session 2026-08-25 — rallen7425@gmail.com set up as main/admin account; real content-based zone classification

Two unrelated pieces of work, both surfaced while investigating a shared-Supabase-project outage that turned out to be unrelated to this app (see `rocky-coast-labs`' own session notes for that incident — resolved via a project Pause+Restore, not anything in this repo).

**Part 1: `rallen7425@gmail.com` set up as the real Distilled account, per explicit direction that this (not the `+distilled`-tagged test account) should be the main/admin account going forward.** This auth.users row had been created directly on the backend for a different app (Rocky Coast Guide admin) and had never been through Distilled's own signup/OAuth callback flow, so it was missing its `distilled.users` profile row entirely — every zone-toggle attempt was silently failing a foreign-key check (`zones.user_id references distilled.users(id)`), which read to the user as "all Zone toggles are broken" (zip/industry prompts hung on "Turn On"; simple zones flashed an unreadable error toast). Root-caused by tracing the exact insert path (`addZone` → `createZoneFromTemplate`) and confirming via direct query that `distilled.users` had zero rows for this id. **Fix:** a plain SQL insert of the missing profile row — not a migration of the old `+distilled` account's data (explicitly declined) and not routed through Google OAuth (an earlier proposed fix that was correctly pushed back on as an unnecessary workaround for what's really a one-row data gap). Also fixed a pre-existing data bug this uncovered: the old `+distilled` account's `distilled.users.email` had never been updated to match its real auth email, which collided with the new insert on the table's `unique(email)` constraint — corrected in place. Verified: zone toggling now works end-to-end for `rallen7425@gmail.com` (created 4 zones — local, sports, news, tech — confirmed live in the DB).

**Part 2: replaced source-based zone classification with real per-article content classification.** Reported directly by the user with two concrete examples: a Dolly Parton obituary (found via Hacker News's general-interest frontpage) was tagged "Tech Zone" purely because HN found it; a Wired piece on a Supreme Court mail-in-ballot ruling was tagged "Tech" purely because it came from Wired's general feed. Traced to the actual root cause — `zone_type` was assigned per-source-feed in `ZONE_RUNNERS` (`scripts/pipeline/index.ts`), never from article content — via an Explore-agent inventory of every `zone_type` read/write site in the app (~30 files). Per the user's explicit product rules (Tech/Sports/Finance/Local/Work have real definable criteria; News is the general catch-all and should not absorb other zones' content *unless* a story is genuinely breaking/critical, e.g. Dolly Parton's death is natively Entertainment but its magnitude also makes it News), planned and shipped:
- Classification moved into the existing per-article Claude call (`scripts/pipeline/enrich/summarize.ts`, already ran for summary/urgency/tags — no new API cost) — the model now classifies each article's real native zone from headline/content alone, ignoring the source hint.
- News-crossover is enforced in code, not left to the model: if the classified zone isn't already `news` and urgency hits 5 (the existing "breaking/critical" rubric tier), `zone_types` becomes `[nativeZone, 'news']`.
- New `distilled.articles.zone_types text[]` column (migration `20260825000002_distilled_multi_zone_articles.sql` in `rocky-coast-labs`) holds every zone an article should be findable under; the existing singular `zone_type` is unchanged and still drives story-detail routing and the single `ZonePill` — only `lib/db/articles.ts`'s three zone-query functions (`getArticlesByZone`, `getTopArticles`, `searchArticlesByTopic`) read the new column, via `.contains`/`.overlaps` instead of `.eq`/`.in`. Existing rows were backfilled to a one-element array matching their current value (schema consistency only — **not** a reclassification of historical content, explicitly scoped out per the user's own choice).
- Verified via direct calls to `summarizeArticles` with the exact two reported examples plus a genuine tech story as a control: Dolly Parton → `entertainment` + `news` (urgency 5, crossover fired correctly); the Supreme Court story → `news` only (correctly out of Tech); a real Apple/iPhone story → `tech` only, urgency 3, no unwanted crossover. `npm run build` clean.
- Known, accepted behavior change: the Local Zone's geo-scoped fetcher previously stamped every article it found `'local'` regardless of topic; a locally-fetched story about (say) a national tech company will now classify as `tech` instead — more correct per the user's stated principle, but the Local Zone will likely show fewer articles than before. No Local-specific carve-out was added.

**Deploy gotcha hit and fixed:** this repo's `.vercel` linkage didn't actually exist locally (despite this file's own "Deploy" section claiming it did) — first `vercel --prod` failed with "Not authorized." Fixed with `vercel link --project projects-yg1p --scope rick-allen-s-projects`, which resolved via `.vercel/repo.json` (a newer Vercel CLI repo-level link, not the classic per-directory `project.json` — that file still doesn't exist here, and that's fine, the repo.json path works). Worth remembering if this recurs.

**Files touched:** `scripts/pipeline/enrich/summarize.ts`, `scripts/pipeline/types.ts`, `scripts/pipeline/write.ts`, `lib/db/articles.ts`, `types/supabase.ts` (regenerated). No UI files touched (by explicit scoping decision — zone pills still show only the native zone). `rocky-coast-labs/supabase/migrations/20260825000002_distilled_multi_zone_articles.sql` (schema), plus a direct one-row `distilled.users` insert + one-row email correction (not a migration file — ad hoc data fixes, run via `supabase db query`).
...
**Committed and deployed** (`7c7d684`, pushed to `origin/main`, `npx vercel --prod --scope rick-allen-s-projects`) — verified live at https://distilled-news.vercel.app (clean sign-in screen, zero console errors) after the Vercel CLI's default scope initially failed to resolve the linked team.
