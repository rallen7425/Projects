...
// A "Story Card" — the expanded, Detailed-view-style tile used on the In-Depth View for Today/More
function StoryCard({
  article,
  isSaved,
  onOpen,
...
export default function InDepthClient({
  breakingArticles,
  topStoryArticles,
  remainingArticles,
  initialSavedIds,
  ...
  // Today paginates through whatever's left after Breaking + Top Stories, 5 at a time
  ...
  const todayArticles = filteredRemaining.slice(0, visibleCount)
  const hasMoreToday = filteredRemaining.length > visibleCount

  const hasAnyArticles = breakingArticles.length > 0 || topStoryArticles.length > 0 || remainingArticles.length > 0

  const renderExpandedArticles = (list: ArticleDisplay[]) => list.map((article) => (
    <StoryCard
      key={article.id}
      article={article}
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100dvh', paddingBottom: '120px' }}>
      ...
      {/* Today — expanded, detail-style cards, paginated 5 at a time via "View More" */}
        ...
        <>
          ...
          {todayArticles.length > 0 ? (
            <div style={{ padding: '0 20px' }}>
              {renderExpandedArticles(todayArticles)}
              {hasMoreToday && (
                <button
            ...
            <div style={{ padding: '0 20px 24px', textAlign: 'center', fontSize: '13px', color: 'var(--text-3)' }}>
