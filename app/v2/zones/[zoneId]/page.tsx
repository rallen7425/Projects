  ...
  return (
    <div>
      ...
      <div className="px-4 pt-3 pb-3 border-b border-[#f0f1f3]" style={{ background: colors.pillBg }}>
        <div className="text-[9px] font-semibold tracking-widest mb-2" style={{ color: colors.pillText, opacity: 0.7 }}>QUICK LOOK</div>
        <div className="flex gap-2 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
          {zone.quickLook.map((chip, i) => (
            ...
            >
              <div className="text-[8px] mb-[3px] font-semibold tracking-wide" style={{ color: colors.pillText, opacity: 0.65 }}>{chip.label}</div>
              <div className="text-[13px] font-semibold leading-snug" style={{ color: colors.dark }}>{chip.value}</div>
              <div className="text-[9px] mt-[2px]" style={{ color: colors.pillText, opacity: 0.6 }}>{chip.sub}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {zone.groups.map((group) => (
        <div key={group.label}>
          <div className="px-4 pt-3 pb-1.5 text-[9px] font-semibold tracking-widest text-[#7a8499] uppercase">{group.label}</div>
            ...
            return (
              <div key={story.id} className="px-4 py-3.5 border-b border-[#f5f5f5]">
                ...
                <div className="flex items-center gap-[7px] mb-1.5">
                  <span
                    className="text-[9px] font-semibold tracking-wide px-[7px] py-[2px] rounded-[4px]"
                ...
                {/* Summary preview */}
                <div className="text-[13px] text-[#555555] leading-relaxed mb-2.5 line-clamp-3">{story.summary}</div>
